...
typedef struct {
    ...
    XOtsu_threshold_top_uio_map maps[ MAX_UIO_MAPS ];
} XOtsu_threshold_top_uio_info;

/***************** Variable Definitions **************************************/
static XOtsu_threshold_top_uio_info uio_info;
...
static int uio_info_read_name(XOtsu_threshold_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XOtsu_threshold_top_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XOtsu_threshold_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XOtsu_threshold_top_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XOtsu_threshold_top_Initialize(XOtsu_threshold_top *InstancePtr, const char* InstanceName) {
	XOtsu_threshold_top_uio_info *InfoPtr = &uio_info;
